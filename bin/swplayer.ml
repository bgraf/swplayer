...
module History = struct 
  ...
  let cache_directory = 
    AppDirs.cache_dir |> Option.value_exn
  ...
  let read () =
    try
      Stdio.In_channel.read_lines history_file
      |> List.rev
    with
    Sys_error _ -> []
end
...
let find_files ~extensions directory = 
  let is_valid_extension ext = 
    List.exists ~f:(String.equal ext) extensions
  in
  ...
  let check_and_normalize dir file =
    ...
    |> Option.map ~f:(fun _ -> Filename.to_absolute_exn ~relative_to:dir file |> normalize_path)
  in
...
let filter_history ~files history =
  ...
  |> List.filter_map ~f:(fun entry -> 
      Hashtbl.find_and_remove table entry
      |> Option.map ~f:(fun _ -> entry))

let run_player file =
  let p = Core_unix.create_process ~prog:"mpv" ~args:[file]  in
  Core_unix.waitpid p.pid

let play_single_file files =
  let history =
    History.read ()
    |> filter_history ~files
  in

  let never_played_files = 
    List.filter ~f:(fun entry -> List.exists ~f:(String.equal entry) history |> not) files
  in

  let never_played_weights = Array.create ~len:(List.length never_played_files) 1.0 in
  let history_weights =
    let n = List.length history in
    let denom_offset = 
      if List.is_empty never_played_files then n else n + 1 
    in
  ...
  let eligible_files = 
    List.concat [never_played_files; history]
    |> List.to_array
  in
  ...
  run_player chosen_file
  |> Result.is_ok

let main no_shutdown count directories = 
  let files =
    directories
  ...
  if List.is_empty files |> not then
    let rec iter i =
      if i < count then
        match play_single_file files with
        | true -> iter (i + 1)
        | _ -> false
      else
        true
    ...
    if (iter 0) && not no_shutdown then
      Core_unix.create_process ~prog:"shutdown" ~args:["-h"; "now"]
      |> ignore
...
let () = 
  ...
  let term = Term.(const main $ no_shutdown $ count $ directories) in
